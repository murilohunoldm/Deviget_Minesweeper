...
def Start():
    ...
    global campo, af, mband, t, bombs, hits, endgame, played, relogio
        ...
        for c in range((x - 1), (x + 2)):
            if ((l>=0 and c>=0) and (l<t and c<t)):
                if(mband[l][c]==0):
                    ...
                    if(campo[l][c]!="0"):
                        screen.blit(Color(campo[l][c]), ((c * comp + (comp / 4)), l * alt))
...
def Play(x, y):
    ...
    if (campo[y][x] !='*'):
        if(campo[y][x]=="0"):
            ...
            screen.blit(Color(campo[y][x]), ((x * comp + (comp / 4)), y * alt))
    else:
        Explode(x, y)
...
def Flags(x, y):
    ...
    if(mband[y][x]==0 and af[y][x]==0):
        if(campo[y][x]=="*"):
            hits+=1
        mband[y][x]=1
        screen.blit(flag, ((int(x) * comp + (comp / 8)), (int(y) * alt + (alt / 8))))
    elif(af[y][x]==0):
        if(campo[y][x]=="*"):
            hits-=1
...
while True:
    for e in pygame.event.get():
        ...
        if ((e.type == MOUSEBUTTONDOWN and pygame.mouse.get_pressed() == (1,0,0) and endgame == False)):
            ...
            if(int(y/alt)<t):
                if(not played):
                    played=Create(int(x / comp), int(y / alt))
                    Play(int(x / comp), int(y / alt))
                    #Starts clock
                    relogio.tick()
